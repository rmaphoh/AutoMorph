...
def create_data_disc_centred_C(cfg):

        
    if os.path.exists('{}M2/artery_vein/Zone_C_disc_centred_artery_skeleton/.ipynb_checkpoints'.format(cfg.results_dir)):
        shutil.rmtree('{}M2/artery_vein/Zone_C_disc_centred_artery_skeleton/.ipynb_checkpoints'.format(cfg.results_dir)) 
    if os.path.exists('{}M2/binary_vessel/Zone_C_disc_centred_binary_skeleton/.ipynb_checkpoints'.format(cfg.results_dir)):
        shutil.rmtree('{}M2/binary_vessel/Zone_C_disc_centred_binary_skeleton/.ipynb_checkpoints'.format(cfg.results_dir)) 
    if os.path.exists('{}M2/artery_vein/Zone_C_disc_centred_vein_skeleton/.ipynb_checkpoints'.format(cfg.results_dir)):
        shutil.rmtree('{}M2/artery_vein/Zone_C_disc_centred_vein_skeleton/.ipynb_checkpoints'.format(cfg.results_dir))
    ...
    Artery_PATH = '{}M2/artery_vein/Zone_C_disc_centred_artery_skeleton'.format(cfg.results_dir)
    Vein_PATH = '{}M2/artery_vein/Zone_C_disc_centred_vein_skeleton'.format(cfg.results_dir)
    Binary_PATH = '{}M2/binary_vessel/Zone_C_disc_centred_binary_skeleton'.format(cfg.results_dir)
    
    for filename in sorted(glob.glob(os.path.join(Binary_PATH, '*.png'))):
        segmentedImage = retina.Retina(None, filename, store_path='{}M2/binary_vessel/Zone_C_disc_centred_binary_process'.format(cfg.results_dir))
        ...
        FD_binary,VD_binary,Average_width,t2, t4, td, vessel_count_list, w1_list, w1_list_average, _, _,_,_ = tortuosity_measures.evaluate_window(window, CONFIG.pixels_per_window, CONFIG.sampling_size, CONFIG.r_2_threshold,store_path='{}M2/binary_vessel/Zone_C_disc_centred_binary_process/'.format(cfg.results_dir))
        #print(window.tags)
    ...
    for filename in sorted(glob.glob(os.path.join(Vein_PATH, '*.png'))):
        ...
        segmentedImage = retina.Retina(None, filename,store_path='{}M2/artery_vein/Zone_C_disc_centred_vein_process'.format(cfg.results_dir))
        #segmentedImage.threshold_image()
        #segmentedImage.reshape_square()
        #window_sizes = segmentedImage.get_window_sizes()
        window_sizes = [912]
    ...
    Data4stage2.to_csv('{}M3/Disc_centred/Disc_Zone_C_Measurement.csv'.format(cfg.results_dir), index = None, encoding='utf8')
