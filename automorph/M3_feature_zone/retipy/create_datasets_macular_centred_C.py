...
def create_macular_centred_C():
    ...
    Artery_PATH = '{}/M2/artery_vein/macular_Zone_C_centred_artery_skeleton'.format(gv.results_dir)
    Vein_PATH = '{}/M2/artery_vein/macular_Zone_C_centred_vein_skeleton'.format(gv.results_dir)
    Binary_PATH = '{}/M2/binary_vessel/macular_Zone_C_centred_binary_skeleton'.format(gv.results_dir)
    
    for filename in sorted(glob.glob(os.path.join(Binary_PATH, '*.png'))):
        segmentedImage = retina.Retina(None, filename, store_path='{}/M2/binary_vessel/macular_Zone_C_centred_binary_process'.format(gv.results_dir))
        ...
        FD_binary,VD_binary,Average_width,t2, t4, td, vessel_count_list, w1_list, w1_list_average, _, _,_,_ = tortuosity_measures.evaluate_window(window, CONFIG.pixels_per_window, CONFIG.sampling_size, CONFIG.r_2_threshold,store_path='{}/M2/binary_vessel/macular_Zone_C_centred_binary_process/'.format(gv.results_dir))
        #print(window.tags)
    ...
    for filename in sorted(glob.glob(os.path.join(Artery_PATH, '*.png'))):
        ...
        segmentedImage = retina.Retina(None, filename,store_path='{}/M2/artery_vein/macular_Zone_C_centred_artery_process'.format(gv.results_dir))
        window_sizes = [912]
        window = retina.Window(
            segmentedImage, window_sizes[-1], min_pixels=CONFIG.pixels_per_window)
        FD_binary,VD_binary,Average_width,t2, t4, td, vessel_count_list, w1_list, w1_list_average,CRAE_Hubbard, _,CRAE_Knudtson,_ = tortuosity_measures.evaluate_window(window, CONFIG.pixels_per_window, CONFIG.sampling_size, CONFIG.r_2_threshold,store_path='{}/M2/artery_vein/macular_Zone_C_centred_artery_process/'.format(gv.results_dir))
        #print(window.tags)
    ...
    for filename in sorted(glob.glob(os.path.join(Vein_PATH, '*.png'))):
        ...
        segmentedImage = retina.Retina(None, filename,store_path='{}/M2/artery_vein/macular_Zone_C_centred_vein_process'.format(gv.results_dir))
        #segmentedImage.threshold_image()
        #segmentedImage.reshape_square()
        #window_sizes = segmentedImage.get_window_sizes()
        window_sizes = [912]
    ...
    Data4stage2.to_csv('{}/M3/Macular_centred/Macular_Zone_C_Measurement.csv'.format(gv.results_dir), index = None, encoding='utf8')